...
# UI elements
title = tk.Label(window, text="Face Recognition Based Attendance Management System",
                 bg="black", fg="white",  height=3, font=('times', 30, 'bold'))

title.pack(fill='both')

lbl_id = tk.Label(window, text="Enter ID", width=20, height=2,
                  fg="white", bg="black", font=('times', 15, ' bold '))
lbl_id.place(x=250, y=200)

txt_id = tk.Entry(window, width=20, bg="#abb0b0",
                  fg="black", font=('times', 25, ' bold '))
txt_id.place(x=500, y=205)

lbl_name = tk.Label(window, text="Enter Name", width=20, fg="white",
                    bg="black", height=2, font=('times', 15, ' bold '))
lbl_name.place(x=250, y=300)

txt_name = tk.Entry(window, width=20, bg="#abb0b0",
                    fg="black", font=('times', 25, ' bold '))
txt_name.place(x=500, y=305)

lbl_notification = tk.Label(window, text="Status : ", width=20,
                            fg="white", bg="black", height=2, font=('times', 15, ' bold  '))
lbl_notification.place(x=250, y=400)

txt_notification = tk.Label(window, text="", bg="#abb0b0", fg="blue", width=60,
                            height=2, activebackground="yellow", font=('times', 15, ' bold '))
txt_notification.place(x=500, y=400)


# functions to clear entry
def clear():
    txt_id.delete(0, 'end')
    res = ""
...
def TakeImages():
    ...
    if(name.isalpha() and not IdExists(Id)):
        ...
        while(True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.putText(img, "Sample taken:" + str(sampleNum),
            ...
            # wait for 100 miliseconds or till 'q' is pressed
...
# function to check if an id already exist
def IdExists(id):
    ID = str(id)
    with open('StudentDetails/StudentDetails.csv', 'r') as file1:
...
clearButton = tk.Button(window, text="Clear", command=clear, fg="white", bg="black",
                        width=20, height=2, activebackground="Red", font=('times', 13, ' bold '))
...
view = tk.Button(window, text="Attendance", command=viewAttendance, fg="white", bg="black",
                 width=20, height=3, activebackground="Red", font=('times', 15, ' bold '))
view.place(x=550, y=615)
